...
public final class SCTMExecutor extends Builder implements SimpleBuildStep {
	...
	public SCTMExecutor(final int projectId, final String execDefIds, final int delay, final int buildNumberUsageOption,
			final String jobName, final boolean contOnErr, final boolean collectResults,
			final boolean ignoreSetupCleanup, boolean useSpecificInstance, String specificServiceURL,
			String specificUser, String specificPassword) {
	...
	public boolean perform(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener)
		...
		return perform(build, build.getWorkspace(), build.number, listener);
	}

	private boolean perform(AbstractBuild<?, ?> build, FilePath workspace, int thisBuildNumber, TaskListener listener)
		...
		try {
			...
			FilePath rootDir = createResultDir(thisBuildNumber, workspace, listener);

			Collection<Thread> executions = new ArrayList<Thread>(execDefIdList.size());
			int buildNumber = getOrAddBuildNumber(thisBuildNumber, build, listener, execDefIdList.get(0), service);
			for (Integer execDefId : execDefIdList) {
				...
				Runnable resultCollector = new ExecutionRunnable(service, execDefId, buildNumber, resultWriter,
						listener.getLogger());
			...
			LOGGER.log(Level.SEVERE,
					MessageFormat.format("Creating a remote connection to SCTM host ({0}) failed.", serviceURL), e); //$NON-NLS-1$
	...
	private int getOrAddBuildNumber(int thisBuildNumber, AbstractBuild<?, ?> build, TaskListener listener, int nodeId, ISCTMService service)
		...
		case OPT_USE_SPECIFICJOB_BUILDNUMBER:
			...
			if (buildNumberUsageOption == OPT_USE_THIS_BUILD_NUMBER) {
				buildnumber = thisBuildNumber;
			} else if (buildNumberUsageOption == OPT_USE_SPECIFICJOB_BUILDNUMBER) {
				if (build == null) {
					throw new IllegalArgumentException(
							"UseSpecificJobBuildNumber not supported for Pipeline scripts yet");
				}
	...
	public void perform(Run<?, ?> run, FilePath filePath, Launcher launcher, TaskListener listener)
			throws InterruptedException, IOException {
		boolean success = perform(null, filePath, run.number, listener);
		if (!success) {
			throw new AbortException(Messages.getString("SCTMExecutor.log.failure"));
		}
	}
}
